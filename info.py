...
BOT_TOKEN = environ.get('BOT_TOKEN', "7936662852:AAGS_u4jtDunV9p-5j_cWJG-LMPurV51gDk")
...
CODE = (environ.get('CODE', 'https://envs.sh/pB9.jpg'))


# Admins, Channels & Users
ADMINS = [int(admin) if id_pattern.search(admin) else admin for admin in environ.get('ADMINS', '7364818327').split()]
CHANNELS = [int(ch) if id_pattern.search(ch) else ch for ch in environ.get('CHANNELS', '-1002277927856').split()] #Channel id for auto indexing ( make sure bot is admin )
...
auth_channel = environ.get('AUTH_CHANNEL', '-1002277927856') #Channel / Group Id for force sub ( make sure bot is admin )
...
CHNL_LNK = environ.get('CHNL_LNK', 'https://t.me/avengerdoomsday1')
OWNER_LNK = environ.get('OWNER_LNK', 'https://t.me/Patrick_Bateman_r')


auth_users = [int(user) if id_pattern.search(user) else user for user in environ.get('AUTH_USERS', '').split()]
AUTH_USERS = (auth_users + ADMINS) if auth_users else []
PREMIUM_USER = [int(user) if id_pattern.search(user) else user for user in environ.get('PREMIUM_USER', '7364818327').split()]
...
MSG_ALRT = environ.get('MSG_ALRT', 'Don't Worry, I'm the Strongest 😉)
...
FILE_STORE_CHANNEL = [int(ch) for ch in (environ.get('FILE_STORE_CHANNEL', '-1002285348355')).split()]
